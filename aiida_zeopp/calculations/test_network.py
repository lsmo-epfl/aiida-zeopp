...
def test_submit_HKUST1(network_code, basic_options):  # pylint: disable=unused-argument,invalid-name,invalid-name
    ...
    structure = CifData(file=os.path.join(TEST_DIR, "HKUST-1.cif"), parse_policy="lazy")
    ...
    cssr = open(os.path.join(TEST_DIR, "HKUST-1.cssr"), encoding="utf8").read()
...
def test_submit_MgO(network_code, basic_options):  # pylint: disable=unused-argument,invalid-name
    ...
    structure = CifData(file=os.path.join(TEST_DIR, "MgO.cif"), parse_policy="lazy")
    ...
    assert pytest.approx(node.outputs.output_parameters.get_attribute("Density"), 0.01) == 3.18223
...
def test_filename(network_code, basic_options):  # pylint: disable=unused-argument,invalid-name
    ...
    input_filename = cif.filename if cif.filename.endswith(".cif") else cif.filename + ".cif"
